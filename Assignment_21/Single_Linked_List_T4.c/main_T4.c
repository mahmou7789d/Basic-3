...
Target      :   C Structure
Name        :   Assignment 21 - Lecture 21 - Problem number 4
Program     :   Write a C function to insert a node at a certain position in a single linked list.
...
int main()
{
    sint32 position,newnode;

    SNode_t* Start=SingleList_CreateNodeList();
    if( Start != NULL)
    {
        printf("\nplease enter position to insert node : ");
        scanf("%d",&position);
        printf("\nplease enter newnode value : ");
        scanf("%d",&newnode);

        SingleList_InserNodeAtCertainPos(&Start,position,newnode);
