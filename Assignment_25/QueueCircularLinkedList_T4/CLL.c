...
CNode_t* CircularList_InsertBeforeNodeWithParticularValue(CNode_t *Last, sint32 ParticularValue, sint32 NewNodevalue)
{
    if (Last == NULL)
    {
        ...
        newNode->Link = newNode;  // Circular reference to itself
        return newNode;  // Return the new node as the new Last
    }

    CNode_t *prev = Last;
    CNode_t *current = Last->Link;

    do
    {
        if (current->Info == ParticularValue)
        {
            CNode_t *newNode = (CNode_t*)malloc(sizeof(CNode_t));
            if (newNode == NULL)
            {
